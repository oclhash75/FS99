...
elseif ($ip == "fix")
  {
    ...
    echo $bold . $fgreen . "[i] Job finished successfully! Please Restart FS99 \n";
    exit;
  }
    ...
    if (!in_array($scan, array(
    ...
    else
      {
        ...
        elseif ($scan == "8")
          {
            ...
            if (count($array) > 0)
              {
                userinput("Do You Want FS99 To Detect CMS Of The Sites? [Y/N]");
                ...
                if ($wp_version != "")
                  {
                    ...
                    if (strpos($vuln_json, "The page you were looking for doesn't exist (404)") !== false)
                      {
                        echo $red . "[!] Seems like the version detection messed up preety bad! Please report here: https://github.com/FS99/issues/new\n";
                      }
                    else
                      {
                  ...
                  {
                    $reallink = $ipsl . $ip;
                    echo $red . "Failed \n\n[!] FS99 could not determine the WordPress version of the target!";
                    echo "\n\n";
                    echo $bold . $yellow . "[*] Scanning Complete. Press Enter To Continue OR CTRL + C To Stop\n\n";
                    trim(fgets(STDIN, 1024));
                    goto scanlist;
        ...
        elseif ($scan == 'U' || $scan == 'u')
          {
            echo "\n\n" . $bold . $yellow . "-[ FS99 Update Corner]-\n\n" . $cln;
            echo $bold . "[i] Fetching Stuffs .... \n" . $cln;
            $latestversion = readcontents("https://raw.githubusercontent.com/FS99/version.txt");
            echo $bold . $blue . "[C] Current Version: " . $rhversion . $cln;
            echo "\n" . $bold . $lblue . "[L] Latest Version:  " . $latestversion . $cln;
            if ($latestversion > $rhversion)
              {
                echo $bold . $fgreen . "\n\n[U] Update Available, Please Update Your Version Of FS99 \n" . $cln;
                echo $bold . $white . "    Link: https://github.com/FS99\n\n" . $cln;
              }

            elseif ($rhversion == $latestversion)
              {
                echo $bold . $fgreen . "\n[i] You are already running the latest version of FS99. \n\n" . $cln;
              }
            else
              {
